...
from utils.table import table_exists, parse_fully_qualified_table_name  # <-- use your helpers
...
# =========================
# YAML loading (expects a list of rule dicts per file)
# =========================
...
def overwrite_rules_into_delta(
    ...
    table_doc: Optional[Dict[str, Any]] = None,  # retained for signature compatibility; ignored here
    *,
    primary_key: str = "check_id",
):
    """
    Overwrite table data on every run. Does not create table, does not touch PK/comments.
    """
...
def main(
    ...
    apply_table_metadata: bool = False,  # <-- new flag
):
    ...
    # Create-once / metadata gating (before any data write)
    ...
    if not exists:
        create_table_if_absent(
            ...
            partition_by=None,  # set to a list if you decide to partition on first create
        )
    elif apply_table_metadata:
        # Re-apply comments only if explicitly requested
    ...
    df = SparkSession.builder.getOrCreate().createDataFrame(all_rules, schema=TABLE_SCHEMA)
    debug_display_batch(spark, df)
    ...
    # OVERWRITE data (PK and comments are handled only as per the create/flag gating above)
...
def load_checks(
    ...
    table_doc: Optional[Dict[str, Any]] = None,
    apply_table_metadata: bool = False,  # <-- expose the flag here too
):
    """
...
res = load_checks(
    ...
    apply_table_metadata=False,  # if True: re-apply table/column comments when table already exists
    # dry_run=True,
    # validate_only=True,
