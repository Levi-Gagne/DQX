...
DQX_CHECKS_LOG_METADATA: Dict[str, Any] = {
    ...
    "table_comment": (
        ...
        "- Each issue in `_errors`/`_warnings` carries its originating `check_id` from the config table.\n"
        "- Fingerprints ignore run_time/user_metadata/check_id for stability.\n"
    ),
    "columns": {
        "log_id": (
        ...
        "_errors_fingerprint": "SHA-256 of a normalized view of `_errors` (ignoring run_time, user_metadata, check_id).",

        "_warnings": (
            "Array<struct> of warning issues. Fields: "
            "{name, message, columns, filter, function, run_time, user_metadata, check_id}."
        ),
        "_warnings_fingerprint": "SHA-256 of a normalized view of `_warnings` (ignoring run_time, user_metadata, check_id).",

        "row_snapshot": (
            "Array<struct{column:string, value:string}> capturing non-reserved columns for the offending row."
        ),
...
def _embed_issue_check_ids(row_log_df: DataFrame, checks_table: str) -> DataFrame:
    ...
    # inline issue struct for casting
    issue_struct_type = T.StructType([
        T.StructField("name",          T.StringType(), True),
        T.StructField("message",       T.StringType(), True),
        T.StructField("columns",       T.ArrayType(T.StringType()), True),
    ...
    def enrich(colname: str) -> DataFrame:
        # explode issues with position (preserve order)
        log_side = (
            row_log_df
        ...
        matched = (
            ...
            .withColumn(
                ...
                ).cast(issue_struct_type)
            )
        )

        # reassemble ordered array
        return (
            matched
            .groupBy("log_id")
            .agg(
                F.array_sort(
                    F.collect_list(F.struct(F.col("pos"), F.col(colname)))
                ).alias("kv")
            )
            .withColumn(colname, F.expr("transform(kv, x -> x."+colname+")"))
            .drop("kv")
        )
