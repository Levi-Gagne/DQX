...
from resources.dqx_functions_0_8_0 import EXPECTED as EXPECTED_ARGS  # spec for coercion
...
def _load_checks_from_table_as_dicts(
    ...
    for r in rows:
        ...
        raw_rules.append({
            ...
            "check": {
                "function":        fn,
                "for_each_column": chk.get("for_each_column") or None,
                "arguments":       args,
            },
        })
...
# ============================================================================
# Row projection & embeddings of check_id
# ============================================================================
...
def run_checks_loader(
    ...
    *,
    ...
    # Resolve checks_config table by name across config
    ...
    for rc_name, _rc_cfg in rc_map.items():
        ...
        # Align to declared schema and write
        write_aligned(spark, out, fqn=results_fqn, write_block={"mode": results_mode, "format": results_fmt, "options": results_opts})
        rows = out.count()
        grand_total += rows
...
# tail of src/dqx/utils/config.py  (additions)

# ----------------------------
# Lightweight helpers (shared)
# ----------------------------
...
# add near TableWriter class or at bottom of src/dqx/utils/write.py
...
def write_aligned(spark: SparkSession, df: DataFrame, *, fqn: str, write_block: Dict[str, Any]) -> None:
    ...
    tw.write_only(
        ...
        format=(write_block.get("format") or "delta"),
        options=(write_block.get("options") or {}),
    )
...
def coerce_arguments(
    ...
    *,
    ...
    """
    Coerce stringy arguments from checks_config to typed values
    according to EXPECTED spec for the given function_name.
    """
