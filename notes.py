...
# NEW: Global default output table for check hits (used if a run-config doesn't specify one)
...
def main(
    ...
    global_results_table = cfg.get("dqx_checks_log_table_name")  # NEW: global fallback
    ...
    for rc_name, rc_cfg in rc_map.items():
        ...
        # precedence: explicit override > per-run-config > global default
        out_loc = (
            results_table_override
            or out_cfg.get("location")
            or global_results_table
        )
        out_mode = out_cfg.get("mode", "overwrite")
        out_options = out_cfg.get("options", {}) or {}

        if not out_loc or str(out_loc).lower() == "none":
